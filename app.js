...
const UI_COPY = Object.freeze({
  ru: {
    ...
    settings: {
      ...
      proTitle: 'Kamu Tales Pro',
      proBody: 'Полный каталог сказок открывается по подписке Kamu Tales Pro.',
  ...
  en: {
    ...
    settings: {
      ...
      proTitle: 'Kamu Tales Pro',
      proBody: 'The full story catalog unlocks with a Kamu Tales Pro subscription.',
  ...
  bg: {
    heroEyebrow: 'Топъл каталог за четене на глас',
    heroTitle: 'Приказки от Каму',
    heroLede: 'Тук живеят добри, вълшебни и мъдри приказки за семейно четене — български и любими истории от цял свят.',
    stats: {
      clean: 'готови',
      audio: 'с аудио',
      visual: 'с илюстрации',
    },
    heroCard: 'Изберете приказка, настанете се удобно и започнете да четете.',
    footerManifest: 'Приказки за семейно четене и уютно слушане.',
    kamuName: 'Каму',
    searchPlaceholder: 'Намерете приказка',
    searchSubmit: 'Показване на филтрите и приказките',
    ageAll: 'Всяка възраст',
    aria: {
      stats: 'Обобщение на каталога',
      heroCard: 'Каму',
      search: 'Търсене на приказки',
      searchInput: 'Намерете приказка',
      ageFilters: 'Възраст за приказките',
      toolbar: 'Библиотека с приказки',
      catalog: 'Каталог с приказки',
    },
    toolbarLabel: 'Библиотека',
    filters: {
      all: 'Всички приказки',
    },
    openStory: (title) => `Отваряне на „${title}“`,
    audioLabel: 'Аудио',
    videoLabel: 'Видео',
    ageLabel: 'Възраст',
    popularityLabel: 'Популярност',
    readTimeLabel: 'Четене',
    charactersLabel: 'Герои',
    readTime: (minutes) => `${minutes} мин четене`,
    loading: 'Зареждаме приказките...',
    loadError: (message) => `Каталогът не можа да бъде зареден: ${message}`,
    empty: 'В този раздел все още няма приказки.',
    settings: {
      title: 'Настройки',
      open: 'Отваряне на настройките',
      close: 'Затваряне на настройките',
      languageTitle: 'Език',
      languageAria: 'Език на приказките',
      proTitle: 'Kamu Tales Pro',
      proBody: 'Пълният каталог с приказки се отключва с абонамент за Kamu Tales Pro.',
      proActive: 'Абонаментът е активен.',
      proInactive: 'Абонаментът не е активен.',
      proLoading: 'Подготвяме абонамента...',
      proUnavailable: 'Абонаментът временно не е достъпен.',
      subscribe: 'Абониране',
...
const UI_LOCALES = Object.freeze(['ru', 'en', 'bg']);
const SEARCH_LOCALES = Object.freeze({ ru: 'ru-RU', en: 'en-US', bg: 'bg-BG' });

const DEFAULT_COLLECTION = Object.freeze({
  id: 'all',
  labels: { ru: 'Сказки', en: 'Fairy tales', bg: 'Приказки' },
  shortLabels: { ru: 'Сказки', en: 'Tales', bg: 'Приказки' },
  uiLanguage: 'ru',
  order: 999,
  accent: '#b85d2b',
});
...
const state = {
  stories: [],
  filters: [{ id: 'all', labels: { ru: 'Все сказки', en: 'All tales', bg: 'Всички приказки' }, uiLanguage: 'ru' }],
  allFilters: [{ id: 'all', labels: { ru: 'Все сказки', en: 'All tales', bg: 'Всички приказки' }, uiLanguage: 'ru' }],
  collections: new Map(),
  activeFilter: 'all',
  activeAge: 'all',
...
function bindAuthCatalog() {
  ...
  initializeSettingsDialog({ onOpen: updateSettingsModal });
}
...
function openSettingsModalForAccountRoute() {
  if (isAccountSettingsRoute()) openSettingsDialog();
}
...
function updateCatalogRuntimeState() {
  ...
  updateSubscriptionManagementLink();
  updateSettingsModal();
}
...
function updateSubscriptionManagementLink() {
  const link = document.querySelector('[data-subscription-management]');
  if (!link) return;
  const subscription = state.backendSession?.subscription || {};
  const managementUrl = safeSubscriptionManagementURL(subscription.managementUrl);
  const visible = Boolean(subscription.active && managementUrl);
  link.hidden = !visible;
  link.setAttribute('aria-hidden', String(!visible));
  link.setAttribute('tabindex', visible ? '0' : '-1');
  link.setAttribute('href', visible ? managementUrl : '#');
}

function safeSubscriptionManagementURL(rawURL) {
  try {
...
function updateSettingsModal() {
  ...
  setText('[data-subscription-management]', copy.manage);
...
function matchesSearchQuery(story) {
  ...
  const locale = SEARCH_LOCALES[currentLanguage()];
  const haystack = [
    story.title,
...
function filtersForStories(filters, stories) {
  ...
  return [{ id: 'all', labels: { ru: 'Все сказки', en: 'All tales', bg: 'Всички приказки' }, uiLanguage: 'ru' }, ...visibleFilters];
}
...
function normalizeUiLocale(value) {
  return UI_LOCALES.includes(value) ? value : DEFAULT_UI_LOCALE;
}
...
function localizedRuntimeHref(webPath) {
  ...
  if (currentLanguage() !== DEFAULT_UI_LOCALE && path.startsWith(prefix)) return path.slice(prefix.length);
  return path;
}
